...
if response.status_code == 200:
    ...
    tentang = soup.find('section', id='Pengenalan')
    if tentang:
        paragraphs = tentang.find_all('p')
        for idx, p in enumerate(paragraphs, 1):
            print(f'Paragraf {idx} dalam Pengenalan:', p.get_text())

    Pendidikan = soup.find('section', id='Pendidikan')
    if Pendidikan:
        paragraphs = Pendidikan.find_all('p')
        for idx, p in enumerate(paragraphs, 1):
            print(f'Paragraf {idx} dalam Pendidikan:', p.get_text())
    
    pengalaman = soup.find('section', id='Pengalaman')
    if pengalaman:
        paragraphs = pengalaman.find_all('p')
        for idx, p in enumerate(paragraphs, 1):
            print(f'Paragraf {idx} dalam Pengalaman:', p.get_text())

    keterampilan = soup.find('section', id='keterampilan')
    if keterampilan:
        paragraphs = keterampilan.find_all('p')
        for idx, p in enumerate(paragraphs, 1):
            print(f'Paragraf {idx} dalam keterampilan:', p.get_text())
    ...
    form = soup.find('aside', id='kontak')
    if form:
